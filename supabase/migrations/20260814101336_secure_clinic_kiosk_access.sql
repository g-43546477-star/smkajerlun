...
-- Rekod klinik hanya boleh digunakan pada kiosk yang telah dilog masuk
-- menggunakan akaun dalam public.admin_pengguna. Jadual kekal dalam schema
-- private dan fungsi RPC menggunakan SECURITY INVOKER supaya RLS sentiasa aktif.
...
alter table private.murid_klinik enable row level security;
alter table private.rekod_klinik enable row level security;

drop policy if exists "Admin kiosk reads clinic students" on private.murid_klinik;
drop policy if exists "Admin kiosk reads clinic visits" on private.rekod_klinik;
drop policy if exists "Admin kiosk registers clinic visits" on private.rekod_klinik;

create policy "Admin kiosk reads clinic students"
  on private.murid_klinik for select to authenticated
  using (exists (select 1 from public.admin_pengguna a where a.user_id = (select auth.uid())));

create policy "Admin kiosk reads clinic visits"
  on private.rekod_klinik for select to authenticated
  using (exists (select 1 from public.admin_pengguna a where a.user_id = (select auth.uid())));

create policy "Admin kiosk registers clinic visits"
  on private.rekod_klinik for insert to authenticated
