...
;
cmsLoadPage('asrama','page-content',{excludeTitles:['Jadual Harian','Peraturan & Pengurusan']});cmsLoadAuthNav();
document.querySelectorAll('.schedule-tab').forEach(function(tab){tab.addEventListener('click',function(){var target=tab.dataset.schedule;document.querySelectorAll('.schedule-tab').forEach(function(item){var selected=item===tab;item.classList.toggle('active',selected);item.setAttribute('aria-selected',String(selected));});document.querySelectorAll('.schedule-panel').forEach(function(panel){panel.hidden=panel.id!==target;});});});
